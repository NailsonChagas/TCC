...
class BuckView(ctk.CTkFrame):
    ...
    def setup_ui(self):
        self.mode_selector = ctk.CTkSegmentedButton(
            self, values=["Parâmetros de Projeto", "Componentes do Circuito"], command=self.handle_mode_switch
        )
        ...
        # ---------------- SOLUÇÃO DE PROTEÇÃO DE ESPAÇO ----------------
        self.content_frame.grid_columnconfigure(0, minsize=400, weight=0) # Esquerda blindada
        self.content_frame.grid_columnconfigure(1, weight=1)              # Direita expansível
    ...
    def handle_mode_switch(self, selected_mode):
        ...
        if selected_mode == "Parâmetros de Projeto": self.param_form_frame.pack(pady=10, padx=10)
        else: self.comp_form_frame.pack(pady=10, padx=10)
    ...
    def refresh_converter_history(self):
        ...
        if not converters:
            ctk.CTkLabel(self.history_scrollable_frame, text="Nenhum conversor guardado.").pack(anchor="w")
            return
    ...
    def handle_save_converter(self):
        try:
            ...
            if self.current_mode == "Parâmetros de Projeto":
                vs = float(self.param_entries['vs'].get().replace(",", "."))
                vo = float(self.param_entries['vo'].get().replace(",", "."))
                ...
                vs = float(self.comp_entries['vs'].get().replace(",", "."))
                vo = float(self.comp_entries['vo'].get().replace(",", "."))
            ...
            if error_msg:
                ...
                success_msg = f"Sucesso! Guardado no Disco.\nRazão Cíclica (D): {buck.D:.2f} | R: {buck.R:.2f} Ω\nL: {buck.L:.6e} H (Mín: {buck.Lmin:.6e} H)"
                self.result_label.configure(text=success_msg, text_color="#2dc937")
                self.refresh_converter_history()
                self.on_converter_added()
        except ValueError:
            self.result_label.configure(text="Erro: Preencha todos os campos com números.", text_color="#ff4a4a")